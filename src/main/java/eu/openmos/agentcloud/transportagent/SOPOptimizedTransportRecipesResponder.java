 ...
 * 
 */
public class SOPOptimizedTransportRecipesResponder extends AchieveREResponder {
    /**
    * Object that writes logs into the file.
    */
    private static final Logger logger = Logger.getLogger(SOPOptimizedTransportRecipesResponder.class.getName());  
     ...
     * listen to.
     */
    public SOPOptimizedTransportRecipesResponder(Agent a, MessageTemplate mt) {
        super(a, mt);
    }
...
//            optimizedRecipes = DeployRecipesRequest.fromString(request.getContent()).getRecipesToBeDeployed(myAgent.getAID());
        ...
        //Call hardware to deploy the recipe
        ...
        if(!recipeActivated){
            ...
            ((TransportAgent) myAgent).activeRecipes.addAll(optimizedRecipes);
